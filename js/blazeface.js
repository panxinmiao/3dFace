...
function scaleBoxFromPrediction(face, scaleFactor) {
    return tf.tidy(() => {
        ...
        return BOX_UTIL.scaleBox(box, scaleFactor).startEndTensor.squeeze();
    });
}
...
class BlazeFace {
    ...
    static async load(path, { maxFaces = 10, inputWidth = 128, inputHeight = 128, iouThreshold = 0.3, scoreThreshold = 0.75 } = {}) {
        const blazefaceModel = await tf.loadGraphModel(path, {fromTFHub: true});
        ...
        for (let i = 0; i < boundingBoxes.length; i++) {
            ...
            const annotatedBox = tf.tidy(() => {
                const box = boundingBox instanceof tf.Tensor ?
                    BOX_UTIL.createBox(boundingBox) :
                    BOX_UTIL.createBox(tf.tensor2d(boundingBox));
                if (!annotateBoxes) {
                    return box;
                }
